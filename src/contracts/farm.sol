...
contract Farm is Ownable {
    ...
    struct PoolInfo 
    {
        ...
        uint256 lastRewardBlock;        // Last block number that FDIC distribution occured.
        uint256 accDinoPerShare;        // Accumulated FDIC per share, times 1e12. See below.
    }

    IERC20 public FDIC;                 // FDIC token
    PoolInfo[] public poolInfo;         // Info of each pool.
    uint256 public fdicPerBlock;        // FDIC tokens created per block.
    uint256 public startBlock;          // The block number at which FDIC distribution starts.
    uint256 public endBlock;            // The block number at which FDIC distribution ends.
    ...
    constructor() public {
        FDIC = IERC20(0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9);
        fdicPerBlock = 2699617876867848400000000000;
        startBlock = block.number;
        endBlock = block.number + 642870;
    }
     ...
     * @param _withUpdate Whether to update all LP token contracts. Should be true if FDIC distribution has already begun.
     */
    ...
    /**
     * @dev Update the given pool's FDIC allocation point. Can only be called by the owner.
    ...
    /**
     * @dev View function to see pending FDIC on frontend.
     * @param _pid ID of a specific LP token pool. See index of PoolInfo[].
     * @param _user Address of a specific user.
     * @return Pending FDIC.
     */
    function pendingFdic(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 dinoReward = multiplier.mul(fdicPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    /**
     * @dev Deposit LP tokens to the Fossil Farm for FDIC allocation.
     * @param _pid ID of a specific LP token pool. See index of PoolInfo[].
    ...
    /**
     * @dev Safe transfer function, just in case if rounding error causes the Fossil Farm to not have enough FDIC.
     * @param _to Target address.
     * @param _amount Amount of FDIC to transfer.
     */
    function safeDinoTransfer(address _to, uint256 _amount) internal {
        uint256 dinoBalance = FDIC.balanceOf(address(this));
        if (_amount > dinoBalance) {
            FDIC.transfer(_to, dinoBalance);
        } else {
            FDIC.transfer(_to, _amount);
        }
    }
    ...
    /**
     * @dev Views total number of FDIC tokens deposited for rewards.
     * @return FDIC token balance of the Fossil Farm.
     */
    function balance() public view returns (uint256) {
        return FDIC.balanceOf(address(this));
    }

    /**
     * @dev Transfer FDIC tokens.
     * @return Success.
     */
    function transfer(address to, uint256 value) external onlyOwner returns (bool) {
        return FDIC.transfer(to, value);
    }

    /**
     * @dev Define last block on which FDIC reward distribution occurs.
    ...
    /**
     * @dev Update FDIC per block.
     * @return FDIC per block.
     */
    function setDinoPerBlock(uint256 _fdicPerBlock) external onlyOwner returns (uint256) {
        require(_fdicPerBlock > 0, "Dino per Block must be greater than 0.");
        massUpdatePools();
        fdicPerBlock = _fdicPerBlock;
        return fdicPerBlock;
    }
